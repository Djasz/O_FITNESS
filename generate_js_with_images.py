...
def generate_js_content(image_dir):
    image_list = []
    # List files in the source directory
    for filename in sorted(os.listdir(image_dir)):
        if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
            # Assume optimization script creates .webp versions
            base_name = os.path.splitext(filename)[0]
            webp_name = f"{base_name}.webp"
            # Path relative to web root
            image_path = f"images/Project_3_Optimized/{webp_name}"
...
source_dir = r"c:\Users\Mr_Komlan\.gemini\antigravity\scratch\OFITNESS\images\Project_3"
images_js_array = generate_js_content(source_dir)

# Read existing portfolio.js content (or use a template if replacing completely)
# Ideally I would read the file, find the project3 entry, and replace its images array.
# For simplicity and robustness, I will rewrite the whole file with the new array inserted.
...
with open(r"c:\Users\Mr_Komlan\.gemini\antigravity\scratch\OFITNESS\js\portfolio.js", "w", encoding="utf-8") as f:
    f.write(js_content)
